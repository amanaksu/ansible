	...
	then
		...
		if [ `grep -i "password" $SYSAUTH | grep "requisite" | grep "lcredit" | grep "dcredit" | grep "ocredit" | wc -l` -eq 0 ]
			then
				...
				echo "[방안]" >> $RESULT_FILE 2>&1
				echo "minlen = 8" >> $RESULT_FILE 2>&1
				echo "lcredit    = -1" >> $RESULT_FILE 2>&1
				echo "ucredit    = 0" >> $RESULT_FILE 2>&1
				echo "dcredit    = -1" >> $RESULT_FILE 2>&1
				echo "ocredit    = -1" >> $RESULT_FILE 2>&1
				echo "패스워드 로그인 시 적용." >> $RESULT_FILE 2>&1
				echo "Pem 사용만 할 시 제외" >> $RESULT_FILE 2>&1
				echo [2-END] >> $RESULT_FILE 2>&1
				echo >> $RESULT_FILE 2>&1
...
Var_3=`cat /etc/pam.d/system-auth | grep "pam_tally" | grep "deny" | wc -l`
if [ $Var_3 -eq 1 ]
	then
		echo "★ system-auth 파일에 임계값 설정이 적용되어 있음" >> $RESULT_FILE 2>&1
		echo "[현황]" >> $RESULT_FILE 2>&1
		...
		echo "★ /etc/pam.d/system-auth 파일에 임계값 설정이 적용되어 있지 않음" >> $RESULT_FILE 2>&1
		echo [3-END] >> $RESULT_FILE 2>&1
		echo >> $RESULT_FILE 2>&1			
	...
	then
		if [ `cat tmp_6_1.txt | grep "^........--" | wc -l` -eq 0 ]
			then
				...
				echo "[방안]" >> $RESULT_FILE 2>&1
				echo "wheel 그룹사용, wheel그룹에 su 권한 부여, wheel 그룹에 사용하는 계정할당 " >> $RESULT_FILE 2>&1
				echo "/etc/pam.d/su 내 wheel 관련 주석 제거하여 wheel그룹사용 및 권한 부여" >> $RESULT_FILE 2>&1
				echo "su 관련 참고 자료 https://access.redhat.com/solutions/64860" >> $RESULT_FILE 2>&1
...
if [ `grep "PASS_MAX_DAYS" /etc/login.defs | grep -v "#" | wc -l` -eq 0 ]
	then
		...
		if [ `grep "PASS_MAX_DAYS" /etc/login.defs | grep -v "#" | awk '{print $2}'` -gt 95 ]
			then
				RE_8_1=0
		fi
fi
...
cat /etc/passwd | egrep "$tshells"  > tmp_10_0.txt

# 서비스 계정을 제외한 사용자 계정명을 추출한다. 
except="service"
awk -F ':' '$2 >= 500 || $2 == 0 ' tmp_10_0.txt  | awk -F ':' '{print $1}' | grep -v $except > tmp_10_1.txt
...
if [ -f /etc/hosts.deny ]
        then
                if [ `cat /etc/hosts.deny | grep -v "^#" | grep -iE "(^sshd|^all) *: *all$" | wc -l` -eq 0 ]
                        then
				if [ `cat /etc/hosts.deny | grep -v "^#" | grep -iE "(^sshd|^all) *: *" | wc -l` -eq 0 ] 
					then
						...
						echo "★ /etc/hosts.deny 파일에 동일대역의 Deny 설정이 적용됨" >> $RESULT_FILE 2>&1
						echo "[현황]" >> $RESULT_FILE 2>&1
...
echo "===============  Linux Server Security Check  ===============" > $RESULT_FILE 2>&1
echo >> $RESULT_FILE 2>&1
echo "Copyright (c) 2015 Songpartners Co. Ltd. All right Reserved" >> $RESULT_FILE 2>&1
echo >> $RESULT_FILE 2>&1
echo >> $RESULT_FILE 2>&1
...
echo "============================================================" >> $RESULT_FILE 2>&1
echo [2-START] >> $RESULT_FILE 2>&1
Var_2_1_d=`cat /etc/security/pwquality.conf | grep -E "^dcredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_2_1_l=`cat /etc/security/pwquality.conf | grep -E "^lcredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_2_1_o=`cat /etc/security/pwquality.conf | grep -E "^ocredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_2_2=`cat /etc/security/pwquality.conf | grep "^minlen" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`

if [ $Var_2_1_d -ge 1 ] && [ $Var_2_1_l -ge 1 ] && [ $Var_2_1_o -ge 1 ] && [ $Var_2_2 -ge 8 ]
	then
...
echo "============================================================" >> $RESULT_FILE 2>&1
echo [7-START] >> $RESULT_FILE 2>&1
Var_7_1_d=`cat /etc/security/pwquality.conf | grep -E "^dcredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_7_1_l=`cat /etc/security/pwquality.conf | grep -E "^lcredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_7_1_o=`cat /etc/security/pwquality.conf | grep -E "^ocredit" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
Var_7_2=`cat /etc/security/pwquality.conf | grep "^minlen" | grep "=" | awk '{print $3}' | awk -F '-' '{print $NF}'`
...
cat /etc/passwd | egrep "$tshells"  > tmp_10_0.txt
awk -F ':' '$2 >= 1000 || $2 == 0 ' tmp_10_0.txt  | awk -F ':' '{print $1}' | grep -v $except > tmp_10_1.txt
for i in `cat tmp_10_1.txt`; do lastlog -b 90 | grep "^$i "; done > tmp_10_2.txt
if [ `cat tmp_10_2.txt | wc -l` -eq 0 ]
        then
                echo "★ 쉘이 부여된 계정 중 90일 이상 로그인하지 않은 계정이 존재하지 않음" >> $RESULT_FILE 2>&1
...
echo [17-START] >> $RESULT_FILE 2>&1
ls -lL /home | awk '{print $3}' | grep "^[0-9]" > tmp_17_1.txt
for user in `cat tmp_17_1.txt`
	do
		if [ ! -d "/home/${user}" ]
			then
				ls -lLd /home/* | grep -w $i >> tmp_17_2.txt
		fi
	done
